...
if ($_POST['remove_admin']){
	update_admin($_POST['remove_admin']);
}

$q1 = mysql_query("select quarter_short_name from quarter where is_current_quarter = 1");
$r1 = mysql_fetch_assoc($q1);
$q = $r1['quarter_short_name'];

// get status
if(($_POST['admin']||$_POST['quarter'])&&$current_quarter > 0){
...
<form method="post" action="" id='passcode_form'>
	<p>
		<div>
		Please enter the new passcode below:
		</div>
		<div>
		<input type="text" name="passcode" id="passcode_box" class='search_box'/>
		</div>
		<?php
		if ($_POST['passcode']){
		?>
		<div>
		<font color=red>New passcode added</font>:&nbsp;<?php echo $_POST['passcode'];?>
		</div>
		<?php
		}
		else{
	...
	<p>
		<div>
		Add a new quarter (format as w02, s02, f02 etc.):
		</div>
		<div>
		<input type="text" name="quarter" id="quarter_box" class='search_box'/>
		</div>
		<?php
		if ($_POST['quarter']){
		?>
		<div>
		<font color=red>New quarter added</font>:&nbsp;<?php echo $_POST['quarter'];?>
		</div>
		<?php
		}
		else{
	...
	<p>
		...
		<br>
		<span style="font-style:italic;font-size:12px;">Plese note that only the person who is/was a collector in the system can be added as admin.</span>
		</div>
		<div>
		...
		?>
		<div>
		<font color=red>New admin added</font>:&nbsp;<?php echo $admin;?>
		</div>
		<?php
		}
		...
		?>
		<div>
		<span style="color:red;font-style:italic">Sorry, we can not locate <?php echo $_POST['admin'];?> in the system. Please try again.</span>
		</div>
		<?php
		}
		?>
